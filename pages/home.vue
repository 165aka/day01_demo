...
const publishProduct = async () => {
  ...
  localStorage.setItem(key, JSON.stringify(myPublishedItems.value));
  // 数据验证
  ...
  const requestBody = {
    ...
    tags: "新品",
    sellerId: userInfo.id, // 优化：使用动态用户 ID
    sellerName: userInfo.name, // 优化：使用动态用户名
    sellerAvatar: userInfo.avatar, // 优化：使用动态用户头像
    location: "校本部",
    isNew: 1,
    isUrgent: 0,
  };
    ...
    if (res.ok) {
      ...
      if (result.code === 20000) {
        ...
        localStorage.setItem(
          "myPublished",
          JSON.stringify(myPublishedItems.value)
        );
...
const loadProducts = async () => {
  ...
  try {
    const res = await fetch(
      "http://localhost:10010/product/search?category=&keyword=&sortBy=newest",
      {
        method: "GET",
        headers: {
          "X-Token": localStorage.getItem("token") || "",
